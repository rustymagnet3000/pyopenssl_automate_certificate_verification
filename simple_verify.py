...
def verify_my_certs():
    try:
        ...
        good_leaf_cert = load_certificate(FILETYPE_PEM, good_leaf_cert_pem)
        bad_leaf_cert = load_certificate(FILETYPE_PEM, bad_leaf_cert_pem)

        assert(isinstance(trusted_certs, X509Store))
        assert(isinstance(bad_leaf_cert, OpenSSL.crypto.X509))
    except:
        print("[!]Failed to load certificates")
        return None
    try:
        store_ctx = X509StoreContext(trusted_certs, bad_leaf_cert)
...
if __name__ == '__main__':
    ...
    print("[*]Verify results: {}".format(verify_my_certs()))
